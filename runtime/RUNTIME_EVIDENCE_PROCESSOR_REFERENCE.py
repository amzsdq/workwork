...
def interval_metrics(p: Probe) -> dict:
    out = {"worked_sec":None,"prearm_overhead_sec":None,"productive_window_sec":None,"close_overhead_sec":None,"productive_marker_pair_valid":False}
    ...
    if p.work_start_marker_created_at and p.pre_close_marker_created_at:
        ...
        out["productive_marker_pair_valid"] = p.work_start_marker_comment_id is not None and p.pre_close_marker_comment_id is not None
...
def base_result(p: Probe) -> dict:
    # target_runtime_sec must survive normalization; derive_bounds cannot infer a lower
    # bound from a clean classification if the target is accidentally discarded.
...
def classify(p: Probe) -> dict:
    ...
    if p.legacy or p.clock_protocol != SERVER_CLOCK:
        return {**base,"result":"LEGACY_SUPPORTING","boundary_effect":"NONE","worked_sec":None,"marker_pair_valid":False,"anomalies":list(p.anomalies)}
    ...
    if p.non_duration_failure:
        ...
        return {**base,"result":"NON_DURATION_FAIL","boundary_effect":"NONE","worked_sec":worked,"marker_pair_valid":marker_ids_complete and marker_times_complete and worked is not None,"anomalies":anomalies}
    if not marker_ids_complete: anomalies.append("MARKER_ID_PAIR_INCOMPLETE")
    if not marker_times_complete: anomalies.append("MARKER_TIMESTAMP_PAIR_INCOMPLETE")
    if not marker_ids_complete or not marker_times_complete:
        return {**base,"result":"CLOCK_EVIDENCE_INVALID","boundary_effect":"NONE","worked_sec":None,"marker_pair_valid":False,"anomalies":anomalies}
    try: worked=worked_sec(p.start_marker_created_at,p.end_marker_created_at)
    ...
    if worked < p.target_runtime_sec:
        if p.close_trigger == "WORKLOAD_EXHAUSTED":
            result="HARNESS_UNDER_TARGET"; anomalies.append("FINITE_WORKLOAD_EXHAUSTION_IS_HARNESS_DEFECT")
        else: result="UNDER_TARGET"
    ...
    elif p.scheduler_write_ok and p.scheduler_state_ok and p.checkpoint_saved and p.clean_close and p.substantive_unit_count > 0:
...
def retrospective_wake(result: dict, wake_observed: bool) -> dict:
    ...
    if result.get("result") == "CLEAN_PASS_PENDING_WAKE" and wake_observed:
        out["result"]="CLEAN_PASS_WAKE_OK"; out["boundary_effect"]="SAFE_LOWER_BOUND_MAY_ADVANCE"
    return out
...
def derive_bounds(classified: Iterable[dict]) -> dict:
    ...
    return {"server_clock_safe_lower_bound_sec":max(clean_targets) if clean_targets else None,"server_clock_failure_boundary_sec":None,"duration_failure_candidate_count":len(failure_candidates),"clean_wake_ok_count":len(clean)}


if __name__ == "__main__":
    p=Probe("REGRESSION-CLEAN",1320,case_id="SC-A22-CLOCK-01",clock_protocol=SERVER_CLOCK,start_marker_comment_id=1,start_marker_created_at="2026-09-23T17:47:04Z",end_marker_comment_id=2,end_marker_created_at="2026-09-23T18:09:06Z",scheduler_write_ok=True,scheduler_state_ok=True,checkpoint_saved=True,clean_close=True,substantive_unit_count=1)
    r=retrospective_wake(classify(p),True)
    assert r["target_runtime_sec"] == 1320
    assert derive_bounds([r])["server_clock_safe_lower_bound_sec"] == 1320
    print(r)
