...
from RUNTIME_EVIDENCE_PROCESSOR_REFERENCE import Probe,SERVER_CLOCK,HARNESS_V2,classify,retrospective_wake
...
def build_probe(row,target_sec=1440):
    start=BASE;end=start+timedelta(seconds=max(0,target_sec+row["target_delta_sec"]));sid=100000+row["ordinal"]*4;eid=sid+3;start_id=sid;end_id=eid;start_ts=z(start);end_ts=z(end);ms=row["marker_state"]
    ...
    scheduler_ok=row["scheduler_state"]=="ok";close_clean=row["close_state"]=="clean";forced=row["close_state"]=="forced_stop";provider_fail=row["provider_state"]=="non_duration_fail";work_start=start+timedelta(seconds=5);pre_close=end-timedelta(seconds=max(1,min(row["close_reserve_sec"],max(1,int((end-start).total_seconds())-6))))
    return Probe(probe_id=row["case_id"],target_runtime_sec=target_sec,case_id="SC-A24-CLOCK+",workload_profile=row["workload_shape"],clock_protocol=SERVER_CLOCK,start_marker_comment_id=start_id,start_marker_created_at=start_ts,work_start_marker_comment_id=sid+1,work_start_marker_created_at=z(work_start),pre_close_marker_comment_id=sid+2,pre_close_marker_created_at=z(pre_close),end_marker_comment_id=end_id,end_marker_created_at=end_ts,harness_version=HARNESS_V2,harness_valid=True,generated_unique_units=1,semantic_unique_units=1,batches_completed=1,scheduler_write_ok=scheduler_ok,scheduler_state_ok=scheduler_ok,checkpoint_saved=close_clean,clean_close=close_clean,forced_stop_or_timeout=forced,non_duration_failure=provider_fail,substantive_unit_count=1,close_trigger="TIMING_ADMISSION")
...
def stratified(seed,per_family=100,target_sec=1440,max_scan=200000):
    families=("NON_DURATION_FAIL","CLOCK_EVIDENCE_INVALID","UNDER_TARGET","DURATION_FAIL_CANDIDATE","AMBIGUOUS","CLEAN_PASS_PENDING_WAKE");selected={f:[] for f in families}
